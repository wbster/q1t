...
export function ease(duration: number, easeFn: (percent: number) => number = (percent) => percent): Operator<number> {
...
export function easeArray(duration: number, easeFn: (percent: number) => number = (percent) => percent): Operator<number[]> {
	return (obs: IObservable<number[]>) => new Observable<number[]>(subscriber => {
